...
| Internal dispatches are unthrottled | runtime | BY DESIGN | The engine's own service identity (`SERVICE_ACTOR_ID`) is excluded from the quota: a consumer dispatching a command is work the system chose to do, so throttling it drops internal work rather than slowing a caller down, and there is no `actors` row to count against. Enforced twice — the guard skips it, and `quota_windows.actor_id references actors (id)` refuses the write. Internal work is bounded by the ledger claims each consumer already makes, not by a per-actor window. |
| A client cannot read its own reaction state | E6 · schema | ABSENT | Phase 69 withheld `reactions.actor_id` from client roles, because a world-readable actor id let anyone enumerate what a given account had reacted to. That closes the leak and leaves a real need unmet: a browser client asking "have I already tapped this" cannot, since referencing a column in a WHERE clause requires SELECT on it. The answer is a `my_reactions` view scoped by `current_actor_id()`, deliberately not built yet — nothing reads it, and a view nobody reads is a surface nobody maintains. Written down here so the day a direct client is added the requirement is known rather than rediscovered. |
| Retention cannot delete remote bytes | E2 · infrastructure | OBJECT_STORAGE_BLOCKED | Phase 65 states the ceilings, decides expiry, marks the rows and writes the ledger. `original_key` is a key into a bucket no environment has configured, so every removal records `object_storage_blocked` rather than claiming a deletion it did not perform. The key column is kept on purpose: forgetting it before the bytes are gone orphans the file permanently. |
